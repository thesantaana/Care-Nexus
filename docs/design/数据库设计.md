...
- 家属绑定使用老人账号标识和独立绑定码；`elder_profile.binding_code_hash` 只保存 BCrypt 哈希。
...
| `care_order_evaluation` | `uk_order_evaluation_order`，每个订单最多评价一次 |
| `care_order_complaint` | `uk_order_complaint_order`，每个订单最多投诉一次 |
